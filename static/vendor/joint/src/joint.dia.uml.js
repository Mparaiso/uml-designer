(function (global) {	// BEGIN CLOSURE
    ...
    /**
     * Predefined aggregation reverse arrow for Class diagram.
     * @name aggregationReverseArrow
     * @memberOf Joint.dia.uml
     * @example c1.joint(c2, Joint.dia.uml.aggregationReverseArrow);
     */
    uml.aggregationReverseArrow = {
        startArrow: { type: "aggregation" },
        endArrow: {type: "none"},
        attrs: { "stroke-dasharray": "none", fill: "white"}
    };
    /**
     * Predefined composition  reverse arrow for Class diagram.
     * @name compositionReverseArrow
     * @memberOf Joint.dia.uml
     * @example c1.joint(c2, Joint.dia.uml.aggregationReverseArrow);
     */
    uml.compositionReverseArrow = {
        startArrow: { type: "aggregation" },
        endArrow: {type: "none"},
        attrs: { "stroke-dasharray": "none", fill: "black"}
    };
    /**
  ...
  methods: ["+createIterator()"]
    ...
    uml.Class = Element.extend({
        ...
        getAutoWidth: function () {
            var max = (+this.inner[0][0].clientWidth);
            var currentWidth;
            for (var i = 1; i < this.inner.length; i++) {
                currentWidth = this.inner[i].getBBox().width;
                if (max < currentWidth) {
                    max = currentWidth;
                }
            }
            return Math.max(this.properties.methodsOffsetX, this.properties.attributesOffsetX, this.properties.labelOffsetX) + max + 5;
        },
        getAutoHeight: function () {
            var h = 0;
            for (var i = 0; i < this.inner.length; i++) {
                h += this.inner[i].getBBox().height;
            }
            return h+this.properties.labelOffsetY+this.properties.swimlane2OffsetY+this.properties.swimlane1OffsetY+this.properties.attributesOffsetY+this.properties.methodsOffsetY;
        },
        init: function (properties) {
            var p = Joint.DeepSupplement(this.properties, properties, {
                attrs: { fill: 'white' },
            ...
            // wrapper
            var wrapper = this.paper.rect(p.rect.x, p.rect.y, p.rect.width, p.rect.height).attr(p.attrs);
            ...
            if (p.autoWidth == true) {
                this.wrapper.attr("width", this.getAutoWidth());
            }
            if (p.autoHeight == true) {
                this.wrapper.attr("height", this.getAutoHeight());
        ...
        getMethodsElement: function () {
            ...
            var
                bb = this.wrapper.getBBox(),
                bbAtrrs = this.inner[1].getBBox(),  // attributes
        ...
        zoom: function () {
            ...
            this.inner[2] = this.getMethodsElement();
            if (this.properties.autoWidth == true) {
                var autoWidth = this.getAutoWidth();
                this.wrapper.attr("width", autoWidth);
            }
            this.inner[3] = this.getSwimlane1Element();
            this.inner[4] = this.getSwimlane2Element();
